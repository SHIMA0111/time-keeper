    ...
    pub(crate) fn is_valid_token(&self, base_time: u64) -> AuthenticateResult<()> {
        if self.exp < self.iat {
            ...
            return Err(AuthenticateError::AccessTokenInvalidException(
                "token setting is wrong. create timestamp is higher than expired time.".to_string()
            ))
        }
        if base_time < self.iat {
            error!("input token will be generated in the future. (base: {}, token_iat: {})", base_time, self.iat);
            return Err(AuthenticateError::AccessTokenInvalidException(
                "token setting is wrong. the create timestamp indicates the future.".to_string()
            ))
        }
        if self.exp < base_time {
            error!("token is expired. (base: {}, token_exp: {})", base_time, self.exp);
            return Err(AuthenticateError::AccessTokenExpiredException(
                format!("The expired time is {}", self.exp)
            ))
        }
